...
public class Combinadas extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource()==this.btNivelUno){
			Juego gm=new Juego(1, "Operaciones combinadas");
			JuegoCombinadas juegoCombinadas = new JuegoCombinadas(1, gm);
			gm.setJuegoCombinadas(juegoCombinadas);
			gm.setLocationRelativeTo(null);
			this.dispose();
		}
		else if(e.getSource()==this.btNivelDos){
			Juego gm=new Juego(2, "Operaciones combinadas");
			JuegoCombinadas juegoCombinadas = new JuegoCombinadas(2, gm);
			gm.setJuegoCombinadas(juegoCombinadas);
			gm.setLocationRelativeTo(null);
			this.dispose();
		}
		else if(e.getSource()==this.btNivelTres){
			Juego gm=new Juego(3, "Operaciones combinadas");
			JuegoCombinadas juegoCombinadas = new JuegoCombinadas(3, gm);
			gm.setJuegoCombinadas(juegoCombinadas);
			gm.setLocationRelativeTo(null);
			this.dispose();
		}
		else if(e.getSource()==this.btNivelCuatro){
			Juego gm=new Juego(4, "Operaciones combinadas");
			JuegoCombinadas juegoCombinadas = new JuegoCombinadas(4, gm);
			gm.setJuegoCombinadas(juegoCombinadas);
			gm.setLocationRelativeTo(null);
			this.dispose();	
		}
		else if(e.getSource()==this.btNivelCinco){
			Juego gm=new Juego(5, "Operaciones combinadas");
			JuegoCombinadas juegoCombinadas = new JuegoCombinadas(5, gm);
			gm.setJuegoCombinadas(juegoCombinadas);
			gm.setLocationRelativeTo(null);
			this.dispose();
		}
